...
from merchants m
join (
    select mid, c.fullname as Customer_Name, sum(price * quantity_available + shipping_cost) as Highest_Spending
    from merchants m
    join sell using (mid)
    ...
    having sum(price * quantity_available + shipping_cost) = (
        ...
        from (
            select mid, c.fullname as Customer_Name, sum(price * quantity_available + shipping_cost) as Total_Spending
            from merchants m
            join sell using (mid)
            join contain using (pid)
            join place using (oid)
...
from merchants m
join (
    select mid, c.fullname as Customer_Name, sum(price * quantity_available + shipping_cost) as Lowest_Spending
    from merchants m
    join sell using (mid)
    join contain using (pid)
    join place using (oid)
    ...
    having sum(price * quantity_available + shipping_cost) = (
        ...
        from (
            select m.mid, c.fullname as Customer_Name, sum(price * quantity_available + shipping_cost) as Total_Spending
            from merchants m
            join sell using (mid)
            join contain using (pid)
            join place using (oid)
